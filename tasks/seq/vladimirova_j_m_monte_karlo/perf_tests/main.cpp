...
TEST(vladimirova_j_m_monte_karlo, test_pipeline_run) {
  // Create data
  ...
  // Create Task
  auto test_task_sequential = std::make_shared<vladimirova_j_m_monte_karlo::TestTaskSequential>(task_data_seq);
  ...
  // std::cout << " ANS " << out[0] << std::endl;
  ASSERT_TRUE((3.14 - out[0]) < 0.005);
}

TEST(vladimirova_j_m_monte_karlo, test_task_run) {
  // Create data
  std::vector<double> val_b = {-1, 1, -1, 1};
  std::vector<double> out(1, 0);
